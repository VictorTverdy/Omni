...
{
	public class HttpDataProvider : DataProvider, IDataProvider
	{
		private byte[] pData;
		...
		public HttpDataProvider(): base()
		{
			CommonHeadersPost = new Dictionary<string, string>();
			CommonHeadersPost["Content-type"] = "application/x-www-form-urlencoded";
		...
		public override void SendRequest(string url, Dictionary<string, string> parameters, Dictionary<string, byte[]> binaryParameters = null)
		{
			...
			if( parameters != null)
			{
				m_wwwRequest = new WWW(Config.URL_BASE + m_url, m_wwwRequestParameters);
			}
			else
			{
				CommonHeadersPost = new Dictionary<string, string>();
				...
				m_wwwRequest = new WWW(Config.URL_BASE + m_url, pData, CommonHeadersPost);
			}
		}
		...
		public override T GetData<T>(ResponseType responseType)
		{
			try
			{
				...
				if(ResponseType.Jpg == responseType){
